...
void CDAGSystem::PopulateSeedEpoch(uint64_t epoch) {
    ...
    for(uint64_t i = start_epoch; i < epoch; i++) {
        ...
        unsigned int* punned = (unsigned int*)seedCache[epoch].data();
...
void CDAGSystem::CreateCacheInPlace(uint64_t epoch) {
    ...
    std::fstream f(std::to_string(epoch) + "dag", std::ios_base::out | std::ios_base::binary);
    f.write((const char*)cacheCache[epoch].data(), cacheCache[epoch].size() * 4);
    f.close();
}
